...
///
/// Ширина кропа — ВСЯ ширина кадра: строки субтитров и интерфейса
/// бывают шире прежних 1440 px, и боковая обрезка резала крайние
/// слова («Scissors» -> «Scisso»), ломая и контекст, и перевод.
///
/// Высота остаётся ограниченной: 900 px при высоте строки ~20-30px
/// вмещают десятки строк, чего с запасом хватает для
/// MAX_SENTENCE_LINES = 12 в extract_sentence_context, и не дают
/// инференсу детектора раздуваться до полноэкранного.
...
pub fn capture_screen(click_x: i32, click_y: i32) -> Result<CapturedScreen, String> {
    ...
    // =========================================================
    //
    // Полоса во всю ширину кадра, по вертикали — окно вокруг клика:
    // строки не теряются по бокам, а объём пикселей остаётся заметно
    // меньше полного монитора.
    //
    // Координаты клика теоретически могут выйти за границы
    // скриншота (многомониторные конфигурации), поэтому вертикаль
    // зажимается в границы изображения. Если клик реально попал за
    // пределы экрана, координата клика в кропе тоже окажется вне
    // кропа — пайплайн отработает как раньше (ошибка
    // "No OCR text region found under cursor").
